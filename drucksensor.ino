...
void updateBuzzer() {
  ...
  const int toneSeq[] = {3000, 2000, 1000};
  const byte toneSeqLen = sizeof(toneSeq) / sizeof(toneSeq[0]);
  ...
  if (!lastAlarmState) {
    toneStep = 0;
    stepStartMs = now;
    tone(PIN_BUZZER, toneSeq[toneStep]);
    lastAlarmState = true;
    return;
  }

  if (now - stepStartMs >= STEP_MS) {
    toneStep++;
    if (toneStep >= toneSeqLen) toneStep = 0;
    stepStartMs = now;
    tone(PIN_BUZZER, toneSeq[toneStep]);
  }
}
